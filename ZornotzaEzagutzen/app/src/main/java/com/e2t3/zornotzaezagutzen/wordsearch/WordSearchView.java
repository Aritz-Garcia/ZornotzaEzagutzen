...
//            Log.d("WordsGrid", getWordStr(cellDragFrom, cellDragTo));
    ...
    private void highlightIfContain(String str) {
        for (Word word : words) {
            if (this.equalsCustom(word.getWord(), str)) {
    ...
    private boolean equalsCustom(String wordtoCompare, String selectedWord) {
        StringBuilder sb = new StringBuilder(selectedWord);
        String inverse = sb.reverse().toString();
        return wordtoCompare.equalsIgnoreCase(selectedWord) || wordtoCompare.equalsIgnoreCase(inverse);
    }
}
